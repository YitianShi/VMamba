...
class BBBLinear(nn.Module):
    def __init__(self, in_features: int, out_features: int, weight_prior, bias_prior, **kwargs):
        ...
        self.bias = GaussianParameter((out_features,)) if self.use_bias else None
...
class BBBLinear2d(nn.Module):
    def __init__(self, in_features: int, out_features: int, weight_prior, bias_prior, **kwargs):
        ...
        if self.use_bias:
            ...
            self.bias = GaussianParameter((self.out_features,))
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.sampling == "parameters":
            ...
            activation_mean = F.conv2d(input, self.weight.mean[:, :, None, None], self.bias.mean)
            activation_var = F.conv2d((input**2).clamp(min=1e-4), (self.weight.std**2).clamp(min=1e-4)[:, :, None, None], (self.bias.std**2).clamp(min=1e-4))
            activation_std = torch.sqrt(activation_var)
...
class BBBConv2d(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, weight_prior, bias_prior, **kwargs):
        ...
        #self.weight_mu = nn.Parameter(torch.empty((self.out_channels, self.in_channels, self.kernel_size, self.kernel_size)).normal_(0, 0.1))
        #self.weight_rho = nn.Parameter(torch.empty((self.out_channels, self.in_channels, self.kernel_size, self.kernel_size)).uniform_(-3, -3))
        self.weight = GaussianParameter((self.out_channels, int(self.in_channels//self.groups), self.kernel_size, self.kernel_size))
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.sampling == "parameters":
            ...
            activation_mean = F.conv2d(input, self.weight.mean, self.bias.mean if self.use_bias else None, stride=self.stride, padding=self.padding, groups=self.groups)
            activation_var = F.conv2d((input**2).clamp(min=1e-4), (self.weight.std**2).clamp(min=1e-4), self.bias.std**2 if self.use_bias else None, stride=self.stride, padding=self.padding, groups=self.groups)
            activation_std = torch.sqrt(activation_var)
...
def make_module_bbb(module, prior):
    ...
    for name, m in list(module._modules.items()):
        if m._modules:
            ...
            bbb_layer = BBBConv2d(in_channels=m.in_channels, out_channels=m.out_channels, kernel_size=m.kernel_size[0], weight_prior=prior, bias_prior=prior, 
                                  stride=m.stride, bias=m.bias is not None, padding=m.padding, groups=m.groups)
        ...
        elif "Linear2d" in m.__class__.__name__:
            bbb_layer = BBBLinear2d(in_features=m.in_features, out_features=m.out_features, weight_prior=prior, bias_prior=prior, use_bias=m.bias is not None)
            bbb_layer.weight.overwrite_mean(m.weight.data.clone())
            if m.bias is not None:
                bbb_layer.bias.overwrite_mean(m.bias.data.clone())
            setattr(module, name, bbb_layer)
