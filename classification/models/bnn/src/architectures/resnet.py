...
def get_linear_layer(in_features, out_features, variational=False, prior=None, rank1=False, components=1):
    if variational:
        if rank1:
...
class ResNet(nn.Module):
    ...
    def _hook_conv(self, idx):
        """
        Creates a hook function that stores the output of the convolutional layer with the given name.
        """
        def hook_function(module, input, output):
            self.feat_conv[f"conv{idx}"] = output.detach()  # Detach the output to prevent tracking history
        return hook_function
    
    def _hook_linear(self):
        """
        Creates a hook function that stores the output of the linear layer with the given name.
        """
        def hook_function(module, input, output):
            self.feat_ln = output.detach()  # Detach the output to prevent tracking history
        return hook_function

    def register_hooks(self):
        """
        Registers hooks on convolutional layers to capture their outputs.
        """
        for i, layer in enumerate(self.named_modules()):
            if isinstance(layer, (nn.Conv2d, Rank1Conv2D, BBBConv2d)):  # Adapt this line based on your layer types
                layer.register_forward_hook(self._hook_conv(idx=i))
                self.conv_layers[f"conv{i}"] = layer
            elif isinstance(layer, (nn.Linear, Rank1Linear, BBBLinear)):
                layer.register_forward_hook(self._hook_linear())
