...
def random_ortho(n, m, std):
    q, _ = torch.linalg.qr(torch.randn(n, m) * std)
    return q


class RandomFourierFeatures(nn.Module):
    """
        Implements random fourier features with orthogonal intialization
    """
    def __init__(self, in_dim, num_random_features, feature_scale=None, std=1.0):
        super().__init__()
        if feature_scale is None:
            feature_scale = math.sqrt(2 / num_random_features)
        ...
        if num_random_features <= in_dim:
            W = random_ortho(in_dim, num_random_features, std)
        else:
            ...
            while dim_left > in_dim:
                ws.append(random_ortho(in_dim, in_dim, std))
                dim_left -= in_dim
            ws.append(random_ortho(in_dim, dim_left, std))
            W = torch.cat(ws, 1)
    ...
    def forward(self, x):
        k = self.feature_scale * torch.cos(x @ self.W + self.b)
        return k
...
class Laplace(nn.Module):
    def __init__(
        ...
        self.rff = RandomFourierFeatures(
            self.num_gp_features, num_random_features, feature_scale, rff_init_std
        )
    ...
    def forward(self, x):
        ...
        # Project input to input dimension of the GP
        f_reduc = self.jl(f)

        # Normalize GP features
        if self.normalize_gp_features:
            f_reduc = self.normalize(f_reduc)

        # Transform to GP feature space
        k = self.rff(f_reduc)
...
class SNGPWrapper(nn.Module):
    '''
        Spectrally-Normalized Gaussian Process
    '''
    def __init__(
        self,
        ...
        self.laplace = Laplace(
            ...
            mean_field_factor=mean_field_factor if sampling_mode == "mean field" else None,
            rff_init_std=rff_init_std
        )
    ...
    def forward(self, x, n_samples=1):
        # Training
        if self.training:
            return self.out_activation(self.laplace(x))
        
        # Evaluation
        if self.sampling_mode == "mean field":
           samples = torch.repeat_interleave(self.out_activation(self.laplace(x)), n_samples, dim=0)
        elif self.sampling_mode == "mc":
            mean, cov = self.laplace(x)
            # Approximating the predictive distribution with a diagonal Gaussian
            # Required because the covariance matrix is often not positive definite (?!)
            predictive_distribution = torch.distributions.Normal(mean, torch.diag(cov).unsqueeze(-1))
            samples = predictive_distribution.rsample((n_samples,))
            samples = self.out_activation(samples)
...
class SNGPOptimizer(BayesianOptimizer):
    def __init__(self, sngp: SNGPWrapper, base_optimizer: torch.optim.Optimizer):
        super().__init__(sngp.parameters(), {})
        self.sngp = sngp
        self.sngp.laplace.reset_precision_matrix()
        self.__base_optimizer = base_optimizer
        self.state["__completed_epoch"] = True

    def step(self, forward_closure, backward_closure, grad_scaler=None):
        if self.state["__completed_epoch"]:
            self.sngp.laplace.reset_precision_matrix()
            self.state["__completed_epoch"] = False
