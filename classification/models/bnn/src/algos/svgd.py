...
class SVGDOptimizer(BayesianOptimizer):
    ...
    def step(self, forward_closure, backward_closure, grad_scaler=None):
        ...
        for particle_idx in range(self.state["__particle_count"]):
            self._set_grad_scaler_state(grad_scaler, torch.cuda.amp.grad_scaler.OptState.READY, self.__base_optimizer)
            ...
            if not self._prepare_and_check_grads(grad_scaler, self.__base_optimizer):
                return None
        ...
        with torch.no_grad():
            ...
            for particle_idx in range(self.state["__particle_count"]):
                ...
                if grad_scaler is not None:
                    self._set_grad_scaler_state(grad_scaler, torch.cuda.amp.grad_scaler.OptState.UNSCALED, self.__base_optimizer)
